% !TeX root = ../../main.tex
% =====================================================================
%  Caminos de carga y diseño de nodos
%  Responsable: 
%  Última revisión: 
% =====================================================================
\chapter{Caminos de carga y diseño de nodos}
\label{ch:caminos_carga_nodos}

\begin{objetivos}
  \item \redactar{Objetivo de aprendizaje 1 --- concreto y comprobable.}
  \item \redactar{Objetivo 2.}
  \item \redactar{Objetivo 3.}
\end{objetivos}

\fichasesion{90 min}{\redactar{capítulos previos}}{\redactar{material}}

\section{Triangulación y transmisión de esfuerzos}
\label{sec:caminos_carga_nodos:01}
\pendiente{Pendiente de redactar.}

\section{Diseño de nodos}
\label{sec:caminos_carga_nodos:02}
\pendiente{Pendiente de redactar.}

\section{Anclajes de suspensión}
\label{sec:caminos_carga_nodos:03}
\pendiente{Pendiente de redactar.}

\section{Refuerzos locales}
\label{sec:caminos_carga_nodos:04}
\pendiente{Pendiente de redactar.}

\section{Errores frecuentes}
\label{sec:caminos_carga_nodos:05}
\pendiente{Pendiente de redactar.}

% ---------------------------------------------------------------------
%  Cierre del capítulo: los cuatro recuadros son obligatorios.
% ---------------------------------------------------------------------

\begin{caso}
\redactar{Qué hicimos nosotros: decisión tomada, datos de partida, resultado en pista y qué haríamos distinto.}
\end{caso}

\begin{ojo}
\begin{itemize}
  \item \redactar{Error que repetimos cada temporada en este tema.}
\end{itemize}
\end{ojo}

\begin{entregable}
\redactar{Qué produce el lector al terminar: plantilla, hoja de cálculo, checklist o apartado del informe.}
\end{entregable}

\begin{juez}
\begin{enumerate}
  \item \redactar{Pregunta tipo del design event sobre este capítulo.}
\end{enumerate}
\end{juez}
