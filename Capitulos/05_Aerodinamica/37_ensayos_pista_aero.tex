% !TeX root = ../../main.tex
% =====================================================================
%  Ensayos aerodinámicos en pista
%  Responsable: 
%  Última revisión: 
% =====================================================================
\chapter{Ensayos aerodinámicos en pista}
\label{ch:ensayos_pista_aero}

\begin{objetivos}
  \item \redactar{Objetivo de aprendizaje 1 --- concreto y comprobable.}
  \item \redactar{Objetivo 2.}
  \item \redactar{Objetivo 3.}
\end{objetivos}

\fichasesion{90 min}{\redactar{capítulos previos}}{\redactar{material}}

\section{Hilos de lana y visualización}
\label{sec:ensayos_pista_aero:01}
\pendiente{Pendiente de redactar.}

\section{Tomas de presión}
\label{sec:ensayos_pista_aero:02}
\pendiente{Pendiente de redactar.}

\section{Coastdown y rectas instrumentadas}
\label{sec:ensayos_pista_aero:03}
\pendiente{Pendiente de redactar.}

\section{Células de carga y altura de paso}
\label{sec:ensayos_pista_aero:04}
\pendiente{Pendiente de redactar.}

\section{Análisis de resultados}
\label{sec:ensayos_pista_aero:05}
\pendiente{Pendiente de redactar.}

% ---------------------------------------------------------------------
%  Cierre del capítulo: los cuatro recuadros son obligatorios.
% ---------------------------------------------------------------------

\begin{caso}
\redactar{Qué hicimos nosotros: decisión tomada, datos de partida, resultado en pista y qué haríamos distinto.}
\end{caso}

\begin{ojo}
\begin{itemize}
  \item \redactar{Error que repetimos cada temporada en este tema.}
\end{itemize}
\end{ojo}

\begin{entregable}
\redactar{Qué produce el lector al terminar: plantilla, hoja de cálculo, checklist o apartado del informe.}
\end{entregable}

\begin{juez}
\begin{enumerate}
  \item \redactar{Pregunta tipo del design event sobre este capítulo.}
\end{enumerate}
\end{juez}
