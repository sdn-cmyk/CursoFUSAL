% !TeX root = ../../main.tex
% =====================================================================
%  Fundamentos de intercambiadores de calor
%  Responsable: 
%  Última revisión: 
% =====================================================================
\chapter{Fundamentos de intercambiadores de calor}
\label{ch:fundamentos_intercambiadores}

\begin{objetivos}
  \item \redactar{Objetivo de aprendizaje 1 --- concreto y comprobable.}
  \item \redactar{Objetivo 2.}
  \item \redactar{Objetivo 3.}
\end{objetivos}

\fichasesion{90 min}{\redactar{capítulos previos}}{\redactar{material}}

\section{Coeficiente global y resistencias en serie}
\label{sec:fundamentos_intercambiadores:01}
\pendiente{Pendiente de redactar.}

\section{Método LMTD}
\label{sec:fundamentos_intercambiadores:02}
\pendiente{Pendiente de redactar.}

\section{Método epsilon-NTU}
\label{sec:fundamentos_intercambiadores:03}
\pendiente{Pendiente de redactar.}

\section{Dónde está el cuello de botella}
\label{sec:fundamentos_intercambiadores:04}
\pendiente{Pendiente de redactar.}

% ---------------------------------------------------------------------
%  Cierre del capítulo: los cuatro recuadros son obligatorios.
% ---------------------------------------------------------------------

\begin{caso}
\redactar{Qué hicimos nosotros: decisión tomada, datos de partida, resultado en pista y qué haríamos distinto.}
\end{caso}

\begin{ojo}
\begin{itemize}
  \item \redactar{Error que repetimos cada temporada en este tema.}
\end{itemize}
\end{ojo}

\begin{entregable}
\redactar{Qué produce el lector al terminar: plantilla, hoja de cálculo, checklist o apartado del informe.}
\end{entregable}

\begin{juez}
\begin{enumerate}
  \item \redactar{Pregunta tipo del design event sobre este capítulo.}
\end{enumerate}
\end{juez}
