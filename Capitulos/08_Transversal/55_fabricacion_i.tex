% !TeX root = ../../main.tex
% =====================================================================
%  Procesos de fabricación I: arranque de viruta y chapa
%  Responsable: 
%  Última revisión: 
% =====================================================================
\chapter{Procesos de fabricación I: arranque de viruta y chapa}
\label{ch:fabricacion_i}

\begin{objetivos}
  \item \redactar{Objetivo de aprendizaje 1 --- concreto y comprobable.}
  \item \redactar{Objetivo 2.}
  \item \redactar{Objetivo 3.}
\end{objetivos}

\fichasesion{90 min}{\redactar{capítulos previos}}{\redactar{material}}

\section{Torneado y fresado}
\label{sec:fabricacion_i:01}
\pendiente{Pendiente de redactar.}

\section{Mecanizado de 3 y 5 ejes}
\label{sec:fabricacion_i:02}
\pendiente{Pendiente de redactar.}

\section{Corte láser, chorro de agua y plegado}
\label{sec:fabricacion_i:03}
\pendiente{Pendiente de redactar.}

\section{Qué pedir y cómo pedirlo}
\label{sec:fabricacion_i:04}
\pendiente{Pendiente de redactar.}

\section{Tolerancias alcanzables}
\label{sec:fabricacion_i:05}
\pendiente{Pendiente de redactar.}

% ---------------------------------------------------------------------
%  Cierre del capítulo: los cuatro recuadros son obligatorios.
% ---------------------------------------------------------------------

\begin{caso}
\redactar{Qué hicimos nosotros: decisión tomada, datos de partida, resultado en pista y qué haríamos distinto.}
\end{caso}

\begin{ojo}
\begin{itemize}
  \item \redactar{Error que repetimos cada temporada en este tema.}
\end{itemize}
\end{ojo}

\begin{entregable}
\redactar{Qué produce el lector al terminar: plantilla, hoja de cálculo, checklist o apartado del informe.}
\end{entregable}

\begin{juez}
\begin{enumerate}
  \item \redactar{Pregunta tipo del design event sobre este capítulo.}
\end{enumerate}
\end{juez}
