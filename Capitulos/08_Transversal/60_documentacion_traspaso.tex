% !TeX root = ../../main.tex
% =====================================================================
%  Documentación y traspaso
%  Responsable: 
%  Última revisión: 
% =====================================================================
\chapter{Documentación y traspaso}
\label{ch:documentacion_traspaso}

\begin{objetivos}
  \item \redactar{Objetivo de aprendizaje 1 --- concreto y comprobable.}
  \item \redactar{Objetivo 2.}
  \item \redactar{Objetivo 3.}
\end{objetivos}

\fichasesion{90 min}{\redactar{capítulos previos}}{\redactar{material}}

\section{Qué se documenta y dónde}
\label{sec:documentacion_traspaso:01}
\pendiente{Pendiente de redactar.}

\section{El handbook del equipo}
\label{sec:documentacion_traspaso:02}
\pendiente{Pendiente de redactar.}

\section{Registros de decisión}
\label{sec:documentacion_traspaso:03}
\pendiente{Pendiente de redactar.}

\section{Documento de traspaso}
\label{sec:documentacion_traspaso:04}
\pendiente{Pendiente de redactar.}

\section{Debrief de fin de temporada}
\label{sec:documentacion_traspaso:05}
\pendiente{Pendiente de redactar.}

% ---------------------------------------------------------------------
%  Cierre del capítulo: los cuatro recuadros son obligatorios.
% ---------------------------------------------------------------------

\begin{caso}
\redactar{Qué hicimos nosotros: decisión tomada, datos de partida, resultado en pista y qué haríamos distinto.}
\end{caso}

\begin{ojo}
\begin{itemize}
  \item \redactar{Error que repetimos cada temporada en este tema.}
\end{itemize}
\end{ojo}

\begin{entregable}
\redactar{Qué produce el lector al terminar: plantilla, hoja de cálculo, checklist o apartado del informe.}
\end{entregable}

\begin{juez}
\begin{enumerate}
  \item \redactar{Pregunta tipo del design event sobre este capítulo.}
\end{enumerate}
\end{juez}
